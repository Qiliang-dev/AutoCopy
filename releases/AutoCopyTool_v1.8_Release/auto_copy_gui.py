...
class AutoCopyApp:
    def __init__(self, root):
        ...
        # Reminder自动消失控制
        self.reminder_auto_close_job = None  # 提醒自动关闭计时器
        self.reminder_auto_hide_enabled = False  # 是否启用自动消失
        self.reminder_auto_hide_seconds = 3  # 自动消失秒数
    ...
    def setup_ui(self):
        ...
        # 收紧第2行（Reminder行）的水平间距：禁止中间列扩展，仅最右列占据剩余空间
        try:
            for col in range(0, 5):
                format_frame.grid_columnconfigure(col, weight=0, minsize=0)
            # 让最右列拉伸，其它列不拉伸，这样中间不会被撑开
            format_frame.grid_columnconfigure(4, weight=1)
        except:
            pass
        ...
        # 将 Reminder Time + Auto hide 放入同一行内联容器，去掉中间空白
        inline = ttk.Frame(format_frame)
        inline.grid(row=2, column=1, columnspan=3, sticky=tk.W, padx=0, pady=1)
        
        self.reminder_time_var = tk.StringVar(value=str(self.reminder_time))
        reminder_entry = ttk.Spinbox(inline, from_=5, to=300, width=4, textvariable=self.reminder_time_var)
        reminder_entry.pack(side=tk.LEFT, padx=(0, 6))
        
        self.reminder_autohide_var = tk.BooleanVar(value=self.reminder_auto_hide_enabled)
        self.reminder_autohide_check = ttk.Checkbutton(inline, text="Auto hide (s)", variable=self.reminder_autohide_var)
        self.reminder_autohide_check.pack(side=tk.LEFT, padx=(0, 6))
        
        self.autohide_seconds_var = tk.StringVar(value=str(self.reminder_auto_hide_seconds))
        self.autohide_seconds_entry = ttk.Spinbox(inline, from_=1, to=60, width=2, textvariable=self.autohide_seconds_var)
        self.autohide_seconds_entry.pack(side=tk.LEFT)
    ...
    def stop_activity_monitoring(self):
        ...
        # 取消提醒自动关闭计时器
        if self.reminder_auto_close_job:
            try:
                self.root.after_cancel(self.reminder_auto_close_job)
            except:
                pass
            self.reminder_auto_close_job = None
    ...
    def show_reminder_dialog(self):
        ...
        # 如果启用自动消失，则在指定秒数后关闭该窗口
        try:
            self.reminder_auto_hide_enabled = bool(self.reminder_autohide_var.get())
        except:
            self.reminder_auto_hide_enabled = False
        if self.reminder_auto_hide_enabled:
            try:
                seconds = int(self.autohide_seconds_var.get())
            except (ValueError, AttributeError):
                seconds = self.reminder_auto_hide_seconds
            self.reminder_auto_hide_seconds = seconds
            self.reminder_auto_close_job = self.root.after(seconds * 1000, self._safe_close_reminder_dialog)
    ...
    def _safe_close_reminder_dialog(self):
        """安全关闭提醒窗口并清理定时器"""
        try:
            if self.reminder_dialog and self.reminder_dialog.winfo_exists():
                self.reminder_dialog.destroy()
...
def main():
    try:
        ...
        # 设置图标
        try:
            root.iconbitmap("resources/icons/autocopy.ico")
        except:
            pass
